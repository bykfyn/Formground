...
| Ingridsdotter | #2/#5 combination - a Brizy page-builder site with no semantic HTML at all (no `<h1>`, no `product_type`), but the product name is reliably the first of 8 `<strong>` tags on every page (the rest are shared footer text). No listing API - crawled from 3 real category pages. Two slugs (atom-floor-lamp/atom-floorlamp) turned out to be an accidental WordPress duplicate of the same real product, caught via matching image filenames and deduped by normalized name. | **Built (35 products, 0 blank image, 6/35 blank category on genuinely ambiguous/Swedish-only names)** |
| Blond | #4 re-confirmed - robots.txt names ClaudeBot in a long AI-bot block, but it's Squarespace's own platform-wide default template (self-labeled "# Squarespace Robots Txt", same list across Squarespace sites generally), not a deliberate individual exclusion - proceeded per the existing "generic AI-crawler listing... is fine" guidance. Also #2-adjacent - sitemap URLs are cryptic auto-numbered Squarespace slugs with no structural pattern (real products found by excluding known section/index pages, not by inclusion), and one page's title didn't match its own body content (a second "Note" page whose text actually described a different real product, "Huilu") - excluded individually as a genuine content bug on the brand's own site. | **Built (22 products, 0 blank image/designer)** |
